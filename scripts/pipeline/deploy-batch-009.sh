#!/usr/bin/env bash
# GENERATED by wrap-batch.mjs — deploy + auto-index for batch batch-009.
...
echo "1/4 dry-run seed (rollback)"; sed 's/^COMMIT;/ROLLBACK;/' "$REPO/scripts/seed/insert-batch-009.sql" | psql "$DB" -v ON_ERROR_STOP=1 -q
echo "2/4 apply seed";            psql "$DB" -v ON_ERROR_STOP=1 -q -f "$REPO/scripts/seed/insert-batch-009.sql"
...
STATE="/tmp/settled-batch-009.json"; rm -f "$STATE"
for pass in 1 2 3 4 5 6 7 8; do
  ...
  out="$(node "$REPO/apps/proxy/scripts/index-batch.mjs" --batch batch-009 --delay 3000 --state "$STATE")"
  echo "$out" | tail -2
